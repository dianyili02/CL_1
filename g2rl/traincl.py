...
# from g2rl.agent import DDQNAgent
# from g2rl.network import CRNNModel
# from g2rl.complexity import compute_map_complexity
...
#     MAP_SETTINGS_PATH = r"C:/Users/MSc_SEIoT_1/MAPF_G2RL-main/g2rl/map_settings_generated.yaml"
#     with open(MAP_SETTINGS_PATH, "r", encoding="utf-8") as f:
#         base_map_settings = yaml.safe_load(f)
...
def _to_tuple_list(x):
    """list[list[int,int]] -> list[tuple[int,int]]，或 None"""
    return None if not x else [tuple(map(int, p)) for p in x]

def _np_grid(g):
    """把任意2D可迭代转成 0/1 np.uint8 网格"""
...
def build_env_from_raw(raw_cfg: dict) -> G2RLEnv:
    """
    过滤只把 G2RLEnv.__init__ 支持的参数传入构造器；
    grid/starts/goals 等信息实例化后挂在 env 上，避免 __init__ 报错。
    """
    sig = inspect.signature(G2RLEnv.__init__)
    allowed = {
        p.name for p in sig.parameters.values()
    ...
    # 将 grid/starts/goals 等作为属性挂载（不进 __init__）
    if "grid" in raw_cfg:
        try:
...
# ============ 基于 Complexity 的 Curriculum Scheduler ============
def _compute_complexities_for_settings(base_map_settings: Dict[str, dict],
                                       size_mode: str = "max") -> pd.DataFrame:
    """
    对每张 YAML 地图直接调用 g2rl.complexity.compute_complexity。
    用返回的 'Complexity' 排序与分桶，并把 FRA/FDA 塞进 spec 方便后续打印/记录。
    """
    rows = []
    for name, spec in base_map_settings.items():
        try:
            grid = _np_grid(spec["grid"])
            starts_t = _to_tuple_list(spec.get("starts"))
            goals_t  = _to_tuple_list(spec.get("goals"))
            agents   = int(spec.get("num_agents", 2))
            ...
            cpx = float(res.get("Complexity", 0.0))
            # 把 FRA/FDA 放进 spec
            spec_with_cpx = dict(spec)
            spec_with_cpx["complexity"] = cpx
            spec_with_cpx["FRA"] = float(res.get("FRA", np.nan))
            spec_with_cpx["FDA"] = float(res.get("FDA", np.nan))

            rows.append({"name": name, "complexity": cpx, "spec": spec_with_cpx})
        except Exception as e:
            rows.append({"name": name, "error": str(e), "spec": spec})
...
def train(
    ...
    while (scheduler is None) or (scheduler.current_stage <= scheduler.max_stage):
        ...
        stage_id = (scheduler.current_stage if scheduler else -1)
        cpx_val = cfg.get("complexity", None)
        fra_val = cfg.get("FRA", None)
        fda_val = cfg.get("FDA", None)

        msg = f"🟢 使用地图：{map_type} | Stage {stage_id} | Agents={env.num_agents}"
        if cpx_val is not None:
            msg += f" | Cpx={float(cpx_val):.3f}"
        if fra_val is not None and not (isinstance(fra_val, float) and np.isnan(fra_val)):
            msg += f" | FRA={float(fra_val):.3f}"
        if fda_val is not None and not (isinstance(fda_val, float) and np.isnan(fda_val)):
            msg += f" | FDA={float(fda_val):.3f}"
        pbar.write(msg)
        ...
        for t in range(timesteps_per_episode):
            ...
            agent.store(
                ...
                reward[target_idx],
                obs[target_idx],
                bool(terminated[target_idx]) if isinstance(terminated, list) else bool(terminated),
            )
            state = obs[target_idx]
        ...
        training_logs.append({
            ...
            "agents": getattr(env, "num_agents", None),
            "complexity": (float(cpx_val) if cpx_val is not None else np.nan),
            "FRA": (float(fra_val) if fra_val is not None else np.nan),
            "FDA": (float(fda_val) if fda_val is not None else np.nan),
            "success": int(success_flag),
            "success_rate": float(success_rate),
        })
...
if __name__ == "__main__":
    ...
    MAP_SETTINGS_PATH = r"C:/Users/MSc_SEIoT_1/MAPF_G2RL-main/g2rl/map_settings_generated_new.yaml"
    with open(MAP_SETTINGS_PATH, "r", encoding="utf-8") as f:
        base_map_settings = yaml.safe_load(f)

    # 顶层为 list 时转成 {name: spec}
    if isinstance(base_map_settings, list):
        base_map_settings = { (m.get("name") or f"map_{i}"): m for i, m in enumerate(base_map_settings) }
    ...
    trained_agent = train(
        ...
        device=device,
        max_episode_seconds=30,
        run_dir="C:/Users/MSc_SEIoT_1/MAPF_G2RL-main/final_trainig_FRA_FDA"
    )
