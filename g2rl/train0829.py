...
# # ===================== 配置 =====================
# # 1) YAML 地图配置
# MAP_SETTINGS_PATH = r"C:/Users/dell/Desktop/CL_1/g2rl/map_settings_generated_new.yaml"

# # 2) 复杂度 CSV（由 infer_complexity.py 生成）
# COMPLEXITY_CSV = r"C:/Users/dell/Desktop/CL_1/0827result/maps_features_with_complexity.csv"
...
# LOG_DIR = "logs"
# RUN_DIR = r"C:/Users/dell/Desktop/CL_1/_refined"
...
# project_root = r"C:/Users/dell/Desktop/CL_1"
...
# -*- coding: utf-8 -*-
"""
train0829_refined.py — 分层课程(细化) + 风险感知 + 拥堵/等待塑形 + 冷却/拉黑 + 兜底避坑
新增：Stage 4 再细分为 4b1 / 4b2，平滑 4a→4b 的难度梯度。
...
# 3) 训练超参（全局默认下限；各子阶段会覆盖）
N_STAGES_BASE = 5                       # 先分 5 桶；再把 2/3/4 分半为 a/b；新增 4b→4b1/4b2
MIN_PER_STAGE = 10
MIN_EPISODES_PER_STAGE = 300
THRESHOLD = 0.80
WINDOW_SIZE = 120
...
def _build_stages_refined(df: pd.DataFrame) -> List[dict]:
    ...
    先按复杂度分 5 桶（0..4）；
    再把 2/3 各自切半为 a/b；
    把 4 桶切为 4a / 4b，最后把 4b 再切半为 4b1 / 4b2：
      0, 1, 2a, 2b, 3a, 3b, 4a, 4b1, 4b2
    """
    dfn = df.dropna(subset=["complexity"]).copy()
    if len(dfn) == 0:
        raise ValueError("没有可用地图用于 complexity 课程（complexity 全 NaN）。")

    qs = np.linspace(0, 1, N_STAGES_BASE + 1)
    ...
    # 2 → 2a/2b
    s2 = buckets[2]
    s2a, s2b = split_half("2a", "2b", s2[3]); stages += [s2a, s2b]

    # 3 → 3a/3b
    s3 = buckets[3]
    s3a, s3b = split_half("3a", "3b", s3[3]); stages += [s3a, s3b]
    ...
    # 4b → 4b1/4b2
    sub4b = pd.DataFrame(s4b["items"])
    if len(sub4b) > 0:
        sub4b = sub4b.sort_values("complexity").reset_index(drop=True)
        mid4b = max(1, len(sub4b) // 2)
        b1 = sub4b.iloc[:mid4b]
        b2 = sub4b.iloc[mid4b:]
        s4b1 = {"name": "4b1", "cpx_min": float(b1["complexity"].min()), "cpx_max": float(b1["complexity"].max()), "items": b1.to_dict("records")}
        s4b2 = {"name": "4b2", "cpx_min": float(b2["complexity"].min() if len(b2) else b1["complexity"].max()),
                "cpx_max": float(b2["complexity"].max() if len(b2) else b1["complexity"].max()),
                "items": b2.to_dict("records")}
    else:
        s4b1 = {"name": "4b1", "cpx_min": s4b["cpx_min"], "cpx_max": s4b["cpx_max"], "items": []}
        s4b2 = {"name": "4b2", "cpx_min": s4b["cpx_min"], "cpx_max": s4b["cpx_max"], "items": []}

    stages += [s4a, s4b1, s4b2]
    return stages
...
class ComplexityScheduler:
    ...
    def __init__(self,
        ...
        self._banned = set()
        self._ban_min_left = 3
        self._ban_soft_stage_idx = 1
        self._long_cooldown = 300

        # 子阶段 agents 上限（新增 4b1/4b2）
        self._agents_cap_by_stage = {
            "2a": 4, "2b": 8, "3a": 8, "3b": 12, "4a": 12, "4b": 16,
            "4b1": 14, "4b2": 16
        }
...
def train(
    ...
    STAGE_OVERRIDES_BY_NAME = {
        ...
        "4b1": {  # 介于 4a 和 4b 之间
            ...
            "target_tau": 0.032,
            "eps_min": 0.18,
            "replay_mix": (0.32, 0.53, 0.15),
            "risk_weight": 0.13,
            "risk_clip": 3.0
        },
        "4b2": {  # 最难
            "max_episode_seconds": 105,
            "batch_size": 128,
            "replay_buffer_size": 65000,
            "target_tau": 0.030,
            "eps_min": 0.16,
            "replay_mix": (0.28, 0.55, 0.17),
            "risk_weight": 0.10,
            "risk_clip": 3.0
        },
    }
    ...
    while True:
        ...
        # 动态 LR（高阶段略升）—— 新增 4b1/4b2
        if hasattr(agent, "optimizer"):
            if not hasattr(agent, "_base_lrs"):
                agent._base_lrs = [pg["lr"] for pg in agent.optimizer.param_groups]
                agent._lr_mode = "base"
            if stage_name in ("3a","3b","4a","4b","4b1","4b2") and agent._lr_mode != "high":
                for g, lr0 in zip(agent.optimizer.param_groups, agent._base_lrs):
                    g["lr"] = lr0 * 1.2
                agent._lr_mode = "high"
                _pwrite(pbar, f"🚀 高阶段LR ×1.2 → {[pg['lr'] for pg in agent.optimizer.param_groups]}")
        ...
        try:
            ...
            if scheduler._ep_in_stage < 100:
                target_idx = min(dists)[1]
            elif stage_name in ("2a","2b","3a","3b","4a","4b","4b1","4b2"):
                target_idx = sorted(dists)[len(dists)//2][1]
            else:
                target_idx = min(dists)[1]
        except Exception:
        ...
        cfg_max = int(cfg.get("max_episode_steps", 10**9))
        env_max = int(getattr(env, "max_episode_steps", cfg_max))
        if stage_name in ("0","1"):
            mult = 8
        elif stage_name in ("2a","2b"):
            mult = 10
        elif stage_name in ("4b1","4b2"):
            mult = 13
        else:
            mult = 12
        timesteps_per_episode = min(env_max, max(80, int(opt_len * mult)))
        ...
        # —— 奖励塑形系数（把 4b1 归入“强一点”组，4b2 归入“弱一点”组）
        if stage_name in ("2a","3a","4a","4b1"):
            K = {"pre_col": -0.20, "wait": 0.06, "osc": -0.06, "time": -0.0005, "crowd": -0.04}
        elif stage_name in ("2b","3b","4b","4b2"):
            K = {"pre_col": -0.14, "wait": 0.04, "osc": -0.04, "time": -0.0007, "crowd": -0.03}
        else:
            K = {"pre_col": -0.12, "wait": 0.03, "osc": -0.03, "time": -0.0010, "crowd": -0.02}
...
if __name__ == "__main__":
    ...
    # 3) 构建 Scheduler（细化后共 9 阶：0,1,2a,2b,3a,3b,4a,4b1,4b2）
    scheduler = ComplexityScheduler(
        base_map_settings=base_map_settings,
        min_per_stage=MIN_PER_STAGE,
        min_episodes_per_stage=MIN_EPISODES_PER_STAGE,
