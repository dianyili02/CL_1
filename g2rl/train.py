...
#     plt.subplot(2, 2, 2)
#     plt.plot(df['Episode'], df['Moving Cost'])
#     plt.title("Moving Cost")

#     plt.subplot(2, 2, 3)
#     plt.plot(df['Episode'], df['Detour Percentage'])
#     plt.title("Detour %")

#     plt.subplot(2, 2, 4)
#     plt.plot(df['Episode'], df['Average Loss'], label='Loss')
#     plt.plot(df['Episode'], df['Average Epsilon'], label='Epsilon')
#     plt.title("Loss & Epsilon")
#     plt.legend()
...
# def train(model, map_name, map_config, num_episodes=10, batch_size=32, device='cuda'):
#     logs = []
#     env = G2RLEnv(**map_config)
#     agent = DDQNAgent(model, env.get_action_space(), device=device)
#     writer = SummaryWriter(log_dir=f'logs/tensorboard_{map_name}_{get_timestamp()}')
#     pbar = tqdm(range(num_episodes), desc=f"{map_name}")
...
#         scalars = {'Reward': 0, 'Moving Cost': 0, 'Detour Percentage': 0, 'Average Loss': 0, 'Average Epsilon': 0, 'Success': 0}
#         success = False
#         retrain_count = 0
...
#                 retrain_count += 1
#                 scalars['Average Loss'] += agent.retrain(batch_size)
#                 scalars['Average Epsilon'] += agent.epsilon
...
#         pbar.set_postfix(success=scalars['Success'])
...
# Main execution
    ...
    # all_logs = []
    # for map_name, map_config in map_settings.items():
    #     print(f"🚀 训练地图 {map_name} | 配置: {map_config}")
    #     model = CRNNModel().to(device)
    #     logs = train(model, map_name, map_config, num_episodes=10, batch_size=32, device=device)
    #     all_logs.extend(logs)

    # df = pd.DataFrame(all_logs)
    # df.to_csv("logs/all_episode_logs.csv", index=False, encoding="utf-8-sig")
    # print("✅ 所有 episode log 已保存到 logs/all_episode_logs.csv")
    # plot_training_results(all_logs, save_path="logs/episode_plot.png")
