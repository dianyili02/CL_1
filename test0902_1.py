...
3) 对冲让行防抖：step 前预测对向对冲，半数 idle
4) 动作掩码（Action Masking）：不越界/不进墙/尽量不撞人
5) 朝目标贪心打破平局 + 不退步（Non-regression）
6) 更强 A* 接管：连续卡住或距离无改善即接管，窗口更长
7) 大图更长步数：size≥96 自动 max_steps_mult=max(传入,6)
8) 易→难混采：解锁后 70% 难图 + 30% 易图
9) 动作顺序锁定：强制按训练顺序 ['idle','up','down','left','right']
10) 兼容两种 step 返回格式：(obs,reward,terminated,truncated,info) 或 (obs,reward,dones,info)

示例（PowerShell 单行）：
python .\test0902.py --weights .\models\best_model.pt --episodes 200 --device cpu --out-dir .\eval_runs --per-episode-seed --assist-astar --max-steps-mult 5 --easy-first
"""
...
def build_env(size: int, num_agents: int, density: float, seed: Optional[int], max_steps_mult: int = 4) -> G2RLEnv:
    """只传 __init__ 支持的键，并尽量覆盖内部 max_episode_steps。大图自动放宽步数。"""
    mult = max_steps_mult
    if size >= 96:
        ...
        with torch.no_grad():
            while True:
                ...
                for i in range(num_agents):
                    ...
                    CANDS.sort(key=lambda nm: goal_distance_after(pos_i, nm, goal_i))  # 靠近目标优先
                    chosen = action_name
                    # 原动作非法/撞人则换更优可行动作
                    if not legal_after(obs[i], action_name, HW) or _is_occupied(obs, apply_delta(pos_i, action_name)):
                        for nm in CANDS:
                            if legal_after(obs[i], nm, HW) and not _is_occupied(obs, apply_delta(pos_i, nm)):
                                chosen = nm
                                break
                    # 不退步：如果拉远目标，尝试不比当前更差的候选；都不行 idle
                ...
                if AStarAgent is not None and (stuck_counter[target_idx] >= K_STUCK or no_improve_cnt >= NO_IMPROVE_K):
...
def parse_args():
    ...
    p.add_argument("--max-steps-mult", type=int, default=4, help="基础规则：max_episode_steps = mult * size；size>=96 自动 max(mult,6)")
    p.add_argument("--easy-first", action="store_true", help="先易后难；解锁后混采（70%难/30%易）")
